...
fn process_instruction(
    ...
    //Getting the accounts to say hello to
    let account = next_account_info(accounts_iter)?;

    //The account must be owned the program ID to execute programs to modify its data
    if account.owner != program_id {
        msg!("Account does not have correct programID");
